...
def get_prompt(major):
    ...
    with open(f"./majors_data/{major}.txt", 'r') as data:
        ...
        prompt += (f"you are now an academic advisor with the purpose of answering any questions given by a student\nYou have this knowledge: \nThe course requirements for a {major} major are:\n")
        prompt += ("--------------------------------------------")
...
def formatq(major, question):
    ...
    with open(f"./majors_data/{major}.txt", 'r') as data:
        ...
        # end of prompt ----
        prompt += ("--------------------------------------------\n")
        prompt += ("Your answers will be based only on the context provided above with no other prior information. All you answers should be formatted as if you were talking to a student directly. Do not mention anything about having the context or list given above.")
